...
#b <-  rbind(a, b)
#}
setwd("~/Documents/R projects/Africa/")
b <- read.csv("Dados África")
...
y <- 0
plot_list = list()
for (i in c(seq(1,length(c[,1])))) {
 axis_x_def <- c[i, 1]
 axis_y_def <- c[i, 2]
 z <- ggplot(d, aes_string(x = axis_x_def, y = axis_y_def)) + geom_point() + 
   ggtitle(paste(axis_x_def,  axis_y_def)) + theme_light()
 plot_list[[i]] = z
 
}

setwd("~/Documents/R projects/Africa/PLOTS")
for (i in seq(1, 468, 1)) {
  ggsave(plot_list[[i]], filename = paste(plot_list[[i]][["labels"]][i], ".PNG"))
}
